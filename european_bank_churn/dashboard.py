...
def _render_high_value(
    ...
    c4.metric("⚠️ Churned balance at risk", f"{premium_kpis['balance_at_risk']:,.2f}")
    st.warning(
...
def _render_model_comparison(data: pd.DataFrame) -> None:
    ...
    c5.metric("⚖️ F1", f"{metrics['f1']:.3f}")
    st.caption(
...
def run_dashboard() -> None:
    ...
    st.markdown(
        ...
        .churnsight-rule {
            ...
            background: linear-gradient(90deg, #FF4B4B, #FF6B6B33);
            border-radius: 2px;
        }
        </style>
